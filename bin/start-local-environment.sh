...
##    - DOCMOSIS_TEMPLATES_ENDPOINT_AUTH: Access key for docmosis templates endpoint.
##
## Start local environment including idam client setup.


# Set variables
COMPOSE_FILE="docker-compose-dependencies.yml"
...
docker-compose -f ${COMPOSE_FILE} up -d shared-db

echo "Starting IDAM(ForgeRock)..."
docker-compose -f ${COMPOSE_FILE} up -d fr-am
docker-compose -f ${COMPOSE_FILE} up -d fr-idm

echo "Starting IDAM..."
docker-compose -f ${COMPOSE_FILE} up -d idam-api
